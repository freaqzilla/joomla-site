...
class DecorHelper {
    ...
    public static function photogallery($items){
        if(preg_match_all('#<img[^>]+>#iU', $items->fulltext, $imgs)){
            //remove the text
            $items->fulltext = preg_replace('#<img[^>]+>#iU', '', $items->fulltext);
            //collect all images
            $img_data = array();
            ...
            if($total > 0) :
                $items->fulltext .=  '<ul class="thumbnails'.($total>1?'':' no-slide').'">';
                $j = 0;
                foreach ($img_data as $img => $attr) :
                  $items->fulltext .=  '<li class="'.($j == 0 ? 'col-xs-12' : 'col-sm-6 col-xs-12').'">'
